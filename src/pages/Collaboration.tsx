...
import { Layers, Zap, Target, Activity as ActivityIcon } from 'lucide-react';
import { clsx } from 'clsx';

const Collaboration: React.FC = () => {
    const context = useContext(AppContext);
    const [activeTab, setActiveTab] = useState<'activities' | 'capabilities' | 'needs' | 'clusters'>('activities');
    ...
    return (
        <div className="pb-20">
            ...
            <div className="max-w-[1200px] mx-auto px-5 lg:px-0">
                ...
                {activeTab === 'clusters' && <ClustersBrowser clusters={data.clusters} entities={data.entities} />}
            </div>
        </div>
    );
};
...
const ClustersBrowser = ({ clusters, entities }: { clusters: any[], entities: any[] }) => {
    const types = Array.from(new Set(clusters.map(c => c.cluster_type))).sort();
    const [selectedType, setSelectedType] = useState<string>('All');
    const [selectedCluster, setSelectedCluster] = useState<string>('All');

    const filteredByType = selectedType === 'All'
        ? clusters
        : clusters.filter(c => c.cluster_type === selectedType);

    const filtered = selectedCluster === 'All'
        ? filteredByType
        : filteredByType.filter(c => c.cluster_name === selectedCluster);

    // Create a map for fast entity lookup with normalized keys
    const entityMap = React.useMemo(() => {
        const map = new Map();
        entities.forEach(e => {
            if (e.entity_name) {
                map.set(e.entity_name.trim(), e);
            }
        });
        return map;
    }, [entities]);
    ...
    return (
        <div className="space-y-8">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                <h3 className="text-xl font-bold text-brand-secondary mb-6">Explore Clusters</h3>
                <div className="grid md:grid-cols-2 gap-6">
                    <div>
                        <label className="block text-sm font-bold text-gray-500 uppercase tracking-widest mb-2">Cluster Type</label>
                        <select
                            className="w-full p-3 rounded-lg border border-gray-200 bg-gray-50 outline-none focus:border-brand-primary transition-colors"
                            value={selectedType}
                            onChange={(e) => {
                    ...
                    <div>
                        <label className="block text-sm font-bold text-gray-500 uppercase tracking-widest mb-2">Cluster</label>
                        <select
                            className="w-full p-3 rounded-lg border border-gray-200 bg-gray-50 outline-none focus:border-brand-primary transition-colors"
                        ...
                        >
                            ...
                            {filteredByType.map((c: any) => (
                                <option key={c.cluster_id} value={c.cluster_name}>{c.cluster_name}</option>
                            ))}
                        </select>
                    </div>
                </div>
            ...
            <div className="grid gap-6">
                {filtered.length === 0 ? (
                    ...
                    filtered.map((cluster: any, idx: number) => {
                        let clusterEntities: string[] = [];
                        try {
                            clusterEntities = typeof cluster.entities === 'string' ? JSON.parse(cluster.entities) : cluster.entities;
                        } catch (e) { clusterEntities = [] }

                        return (
                            <div key={idx} className="bg-white p-8 rounded-xl border border-gray-100 shadow-sm">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h4 className="text-xl font-bold text-brand-secondary">{cluster.cluster_name}</h4>
                                        <span className="text-xs font-bold text-brand-primary uppercase tracking-widest mt-1 block">
                                            {cluster.cluster_type} | {(cluster.confidence * 100).toFixed(0)}% Confidence
                                        </span>
                                    </div>
                                    <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-bold">
                                        {clusterEntities ? clusterEntities.length : 0} Entities
                                    </span>
                                </div>
                                <p className="text-gray-600 mb-6">{cluster.description}</p>

                                {clusterEntities && clusterEntities.length > 0 && (
                                    <div>
                                        <h5 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Entities</h5>
                                        <div className="flex flex-wrap gap-2">
                                            {clusterEntities.sort().map((eName: string, i: number) => {
                                                const normalizedName = eName.trim();
                                                ...
                                                if (url) {
                                                    return (
                                                        <a
                                                            ...
                                                            target="_blank"
                                                            rel="noreferrer"
                                                            className="px-2 py-1 bg-brand-primary/10 text-brand-primary text-xs rounded border border-brand-primary/20 hover:bg-brand-primary hover:text-white transition-colors"
                                                            title={`Visit ${eName}`}
                                                        >
                                                            {eName}
                                                        </a>
                                                ...
                                                return (
                                                    <span key={i} className="px-2 py-1 bg-gray-50 text-gray-600 text-xs rounded border border-gray-200 cursor-default">
                                                        {eName}
                                                    </span>
                                                );
                                            })}
